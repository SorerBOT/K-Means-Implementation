...
def dbscan(points: list[Point], N_0: int, EPS: float):
    ...
    for point in points:
        ...
        for neighboring_point in points_in_neighborhood[point]:
            if len(points_in_neighborhood[neighboring_point]) >= N_0:
    ...
    # Compute connected components of the graph we got (treating points as vertices, where two points are connected by an edge if they share a cluster)
    did_join_clusters = True
    iteration_number = 0
    while did_join_clusters:
        ...
        clusters_new = set(clusters)
        for cluster in clusters:
            for other in clusters.difference({ cluster }):
                if not cluster.isdisjoint(other):
                    new_cluster = cluster | other
                    clusters_new.discard(cluster)
                    clusters_new.discard(other)
                    clusters_new.add(new_cluster) # sometimes, one cluster is a subset of the other. In those cases we're practically removing and re-inserting the same cluster'
                    did_join_clusters = True
